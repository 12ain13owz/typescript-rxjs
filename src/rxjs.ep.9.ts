...
class Subscription {
  ...
  unsubscribe() {
    this.teardownList.forEach((teardown) => {
...
// predicate เป็นชื่อที่นิยมตั้งเป็น Function สำหรับ return ค่าเป็น boolen
    ...
    new Observable((observer) => {
      const subscription = source.subscribe({
        next: (value) => {
          ...
          // if (fn(value) === true) observer.next(value);
          // if (fn(value) !== true) observer.error;
        },
        error: (err) => observer.error(err),
        complete: () => {
          console.log("complete");
